...
#define MALLOC_ACQUIRE()                     \
  do                                         \
  {                                          \
    assert(assertedMultithreadingLock == 0); \
    assertedMultithreadingLock = 1;          \
  } while (0)
#define MALLOC_RELEASE()                     \
  do                                         \
  {                                          \
    assert(assertedMultithreadingLock == 1); \
    assertedMultithreadingLock = 0;          \
  } while (0)
...
static void *attempt_allocate(Region *freeRegion, size_t alignment, size_t size)
{
  ...
  // MAIN_THREAD_ASYNC_EM_ASM(out('+' + Number($0) + ',' + Number($1)), size, usedMemory);
