...
int64_t bufferQueueMemory = 0;

void *mallocProxy(size_t size)
{
  bufferQueueMemory += size;
  return malloc(size);
}

void freeProxy(void *data, int len)
{
  bufferQueueMemory -= len;
  free(data);
}

BufferQueue *bq_init(void)
{
  BufferQueue *queue = mallocProxy(sizeof(BufferQueue));
  queue->pos = 0;
  queue->len = 0;
  queue->max = 8;
  queue->items = mallocProxy(queue->max * sizeof(BufferQueueItem));
  return queue;
}
...
void bq_trim(BufferQueue *queue)
{
  ...
  for (int i = 0; i < queue->len; i++)
  {
    ...
    {
      freeProxy(queue->items[i].bytes, queue->items[i].len);
...
void bq_append(BufferQueue *queue, const char *data, size_t len)
{
  ...
  queue->items[queue->len].bytes = mallocProxy(len);
...
{
  bq_flush(queue);
  freeProxy(queue->items, queue->max * sizeof(BufferQueueItem));
  freeProxy(queue, sizeof(BufferQueue));
};
